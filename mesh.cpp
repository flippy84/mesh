...
int main(int argc, char* argv[])
{
	...
	rcVcopy(t_params[0].bmin, bmin);
	rcVcopy(t_params[0].bmax, bmax);
	t_params[0].size = 0;
	t_params[0].tx = mesh_x;
	t_params[0].ty = mesh_y;
	t_params[0].mesh = m_mesh;

	work = CreateThreadpoolWork(WorkerCallback, &t_params[0], &pcbe);
	SubmitThreadpoolWork(work);
	...
	dtStatus status = m_navMesh->addTile(t_params[0].data, t_params[0].size, DT_TILE_FREE_DATA, 0, 0);
	if(dtStatusFailed(status))
	{
		dtFree(t_params[0].data);
		puts("addTile failed");
	}
